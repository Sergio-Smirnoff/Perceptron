...
def main():
    try:
        ...
        optimizations = ["descgradient", "momentum", "adam"]
        opt_errors = [[] for _ in optimizations]
        for opt_idx, optimizator in enumerate(optimizations):
            perceptron = ParityMultyPerceptron(
                ...
                optimization_mode=optimizator
            )

            for i in range(ERROR_EPOCHS):
                mse = perceptron.train(X_clean, y_digits_clean)
                opt_errors[opt_idx].append(mse)
        
            

        # Resumen por etiqueta de paridad
        with open(os.path.join(OUT_DIR, "gd_adam_momentum.txt"), "w") as f:
            f.write(f"Optimizador;Error promedio MSE ({ERROR_EPOCHS} epochs)\n")
            for opt_idx, optimizador in enumerate(optimizations):
                for mse in opt_errors[opt_idx]:
                    f.write(f"{optimizador};{mse}\n")


        plot_optimization_errors(opt_errors, OUT_DIR)
