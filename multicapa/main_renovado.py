...
def error_vs_epochs_LearningRate_run(X_clean, y):

    optimizer_list =[0.1, 0.01, 0.001, 0.0001]
    perceptron_list = [
        MultiLayer(
            layers_array=[35, LAYER_ONE_SIZE, LAYER_TWO_SIZE, 10],
            learning_rate=opt,
    ...
    for epoch in range(EPOCHS):
        ...
        for i, perceptron in enumerate(perceptron_list):
            error_entropy, error_mse = perceptron.train(X_clean, y)
            error_entropy_list[i].append(error_entropy)
            logger.debug(f"Época {epoch+1}/{EPOCHS} - Error Entropía: {error_entropy:.6f}, Error MSE: {error_mse:.6f}, Learning Rate: {optimizer_list[i]}")

    logger.info("Entrenamiento completado para Error vs Épocas.")
    # Gráfico de error vs épocas
    plt.figure(figsize=(12, 7))
    for i, lr in enumerate(optimizer_list):
        plt.plot(epochs, error_entropy_list[i], label=f'LR={lr}')
    plt.xlabel("Épocas", fontsize=12)
    plt.ylabel("Error", fontsize=12)
    plt.title("Error vs Épocas", fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=10)
    plt.tight_layout()
    plt.savefig(f"{OUT_DIR}/error_vs_epochs_LearningRate_Momentum.png", dpi=300)
    plt.show()
    plt.close()
...
def main():
    try:
        ...
        error_vs_epochs_LearningRate_run(X_clean, y)
